...
@Component({
  selector: 'app-create-catechumene',
  templateUrl: './create-catechumene.component1.html',
  styleUrls: ['./create-catechumene.component1.scss']
})
export class CreateCatechumeneComponent implements OnInit {
  ...
  birthCertificateFileName: string | undefined;
  ...
  onBirthCertificateChange(event: Event): void {
    const input = event.target as HTMLInputElement;
    if (input.files && input.files.length > 0) {
      // Stockez le nom du fichier dans la variable birthCertificateFileName
  ...
  deleteBirthCertificate() {
    // @ts-ignore
    this.register_form.get('kt_birth_certificate').reset(); // Réinitialiser l'input de sélection de fichier
    ...
    return this.catechumene.createCatechumene(formData).subscribe({
      next: (response: any) => {
        ...
        if (this.data && this.data?.kt_uid) {
          ...
          this._router.navigate(['/admin', 'catechumene-received', this.data?.kt_uid]);
          Swal.fire(
            'Success!',
